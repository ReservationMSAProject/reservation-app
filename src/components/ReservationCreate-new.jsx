...
export default function ReservationCreate() {
  ...
  const steps = [
    { id: 1, title: '좌석 선택', icon: TicketIcon },
    { id: 2, title: '예약 정보', icon: UserIcon },
    { id: 3, title: '결제', icon: CheckIcon }
  ];
  ...
  const handleSubmit = async () => {
    ...
    try {
      const reservationPayload = {
        ...
        seatType: reservationData.seatType,
                ...
                >
                  <Card className="p-6">
                    ...
                    <div className="space-y-4">
                      {concert.seatTypes.map((seat, index) => (
                        <motion.div
                          key={index}
                          whileHover={{ scale: 1.02 }}
                          className={`
                            p-4 border-2 rounded-xl cursor-pointer transition-all duration-300
                            ${reservationData.seatType === seat.type
                              ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/20'
                              : 'border-gray-200 dark:border-gray-700 hover:border-primary-300'
                            }
                          `}
                          onClick={() => handleSeatSelection(seat.type, seat.price)}
                        >
                          <div className="flex justify-between items-center">
                            <div>
                              <div className="flex items-center space-x-3">
                                ...
                                <Badge 
                                  variant={seat.available > 50 ? 'success' : seat.available > 10 ? 'warning' : 'danger'}
                                >
                                  {seat.available}석 남음
                                </Badge>
                              </div>
                    ...
                    <div className="mt-6 flex justify-end">
                      <Button
                        ...
                        disabled={!reservationData.seatType}
                      >
                        다음 단계
